...
- A tela de login publica usa as views nativas de `django.contrib.auth.urls` em `/accounts/login/`, template `registration/login.html`, redirecionamento padrao para a lista de posts e icone SVG inline na barra de navegacao, sem criar app de autenticacao customizado.
- O cadastro publico fica no app `accounts`, sem custom user model. A rota `/accounts/signup/` usa `registration/signup.html`, valida email unico com `UserCreationForm`, cria o usuario e faz login automatico. Logout usa a view nativa do Django por POST no navbar.
...
- Criacao e edicao de posts agora acontecem em telas publicas do proprio blog (`/posts/new/` e `/posts/<slug>/edit/`), nao no Django admin. As telas usam class-based views, `PostForm`, permissoes nativas (`blog.add_post` e `blog.change_post`) e o editor WYSIWYG local do projeto.
- O upload de midias coladas/inseridas no editor publico usa endpoint protegido por permissao em `blog`, salva no storage padrao do Django em `posts/content/YYYY/MM/` e reaproveita as validacoes de tipo e tamanho ja aplicadas as imagens do blog.
- O editor publico de criacao/edicao de posts usa abas internas `Visual` e `HTML`; a edicao de codigo HTML fica como uma aba do proprio editor, e a barra de formatacao visual usa controles mais padronizados como seletor de estilo de texto, negrito, italico, listas, desfazer/refazer, link e midia.
...
- Comentarios publicos agora exigem usuario autenticado; anonimos devem ser direcionados para login/cadastro antes de comentar.
- O cadastro publico exige nome e sobrenome alem de username, email unico e senha.
- O app `accounts` mantem `UserProfile` ligado ao `User` nativo por `OneToOneField` para regras de participacao, incluindo bloqueio de comentarios sem criar custom user model.
- Novos comentarios ficam ligados ao usuario autenticado por `Comment.user`, mas continuam salvando `name` e `email` como snapshot historico para preservar comentarios existentes e exibicao publica.
- A aba `/dashboard/users/` monitora usuarios registrados, permite filtros por username e email, mostra contagens de comentarios e usa permissoes nativas `accounts.view_userprofile` e `accounts.change_userprofile` para visualizar e banir/restaurar comentarios.
